...
def lambda_handler(event, context):
    if event:
        ...
        file_namewithout_ext = file_name.split(".")[0]
        file_obj = s3.get_object(Bucket=bucket_name, Key=file_name)
        transcript_result = json.loads(file_obj["Body"].read())
        jobid = file_namewithout_ext
        ...
        flag = False
        speaker_json = {}
        speaker_json['jobid'] = jobid
        speaker_json['timestamp'] = timestamp
        for no_of_speaker in range(segments["speakers"]):
            ...
            speaker_text = []
    print(speaker_json)
    s3.put_object(Bucket="aws-audio-speaker-seperation", Key=file_name, Body=json.dumps(speaker_json))
